...
describe.only('Component > VariableStarViewerContainer', function () {
  ...
  describe('with a subject', function () {
    ...
    describe('setting the series focus', function () {
      it('should default all series focus state to true', function (done) {
        const wrapper = shallow(<VariableStarViewerContainer subject={subject} />)
        expect(wrapper.state().focusedSeries).to.have.lengthOf(0)
        cdmSpy.returnValues[0].then(() => {
          expect(wrapper.state().focusedSeries).to.have.lengthOf(variableStar.data.length)
          variableStar.data.forEach((series, index) => {
            expect(wrapper.state().focusedSeries[index]).to.deep.equal({ [series.seriesOptions.label]: true })
          })
        }).then(done, done)
      })
    })
